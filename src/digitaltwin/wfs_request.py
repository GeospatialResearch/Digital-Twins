...
def access_api_info(engine, source_name: tuple):
    ...
    queries = engine.execute("SELECT data_provider, source_name, layer, source_api, query_dictionary, geometry_col_name\
...
def data_from_apis(engine, key, base_url, layer, geometry_name, table_name, polygon):
    ...
    try:
        ...
        engine.execute('ALTER TABLE \"%(table_name)s\" ADD COLUMN IF NOT\
                            EXISTS unique_id SERIAL PRIMARY KEY' % ({'table_name': table_name}))
...
def get_source_from_db(engine, source_list):
    ...
    query = engine.execute("SELECT source_name, api_modified_date FROM apilinks WHERE source_name IN %(source_list)s",
                           ({'source_list': source_list}))
    sources = []
    for source in query:
        sources.append(source)
    srcListUser_to_dict = pd.DataFrame(sources, columns=['source_name', 'api_modified_date']).to_dict(orient="list")
