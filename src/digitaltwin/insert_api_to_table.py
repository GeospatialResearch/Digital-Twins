...
def region_geometry_table(engine, YOUR_KEY):
    ...
    try:
        ...
        response_data.to_postgis('region_geometry', engine, index=False, if_exists='fail')
    except ValueError:
...
def url_validator(url=None):
    ...
    return url if valid else log.error(f'Invalid URL: {url}')
...
def api_data_modified_date(data_provider: str, url=None):
    ...
    try:
        ...
        if data_provider == 'LINZ' or data_provider == 'LRIS':
            ...
            url_update_date = None
        url_update_date = dmy_to_ymd(url_update_date)
        return url_update_date
    except TypeError:
         log.debug(f'Invalid modified date of the data source {url} from data provider {data_provider}.')
...
def insert_records(engine, data_provider: str, source_name: str, api: str, region_name: str,
                   geometry_col_name: str = None, url=None, layer=None):
    ...
    if source_name[0].isalpha() or source_name[0].startswith("_"):
        ...
        log.error("source_name should start with _ or a letter")
