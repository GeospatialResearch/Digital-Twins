...
def check_table_exist(engine, source_list):
    """To check if tables already exists in the database for the requested sources."""
...
def wfs_request_from_db(engine, tables, polygon): #tables is source_name ('_50329-nz-road-centrelines', '_50319-nz-railway-centrelines')
...
def get_geometry_info(engine):
    ...
    stored_list = engine.execute("SELECT DISTINCT source_list, ST_AsText(geometry) FROM user_log_information")
