...
def clean_rec1_inflow_data(rec1_inflows_w_input_points: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    rec1_inflow_data = rec1_inflows_w_input_points.copy()
    # Select columns starting with 'h' and include 'areakm2', 'river_input_point' and 'dem_resolution'
    rec1_inflow_data = rec1_inflow_data.filter(regex=r'^h|^areakm2$|^river_input_point|^dem_resolution$')
    ...
    rec1_inflow_data.columns = rec1_inflow_data.columns.to_series().replace(column_mapping, regex=True)
    # Move the 'areakm2' column to the last column
    rec1_inflow_data['areakm2'] = rec1_inflow_data.pop('areakm2')
    return rec1_inflow_data


def extract_valid_ari_values(rec1_inflow_data: gpd.GeoDataFrame) -> List[int]:
    """
    Extracts valid ARI (Annual Recurrence Interval) values from the column names of the REC1 river inflow data.

    Parameters
    ----------
    rec1_inflow_data : gpd.GeoDataFrame
        A GeoDataFrame containing REC1 river inflow data with column names that include ARI values.
    ...
    column_names = rec1_inflow_data.columns
...
def get_rec1_inflow_scenario_data(
        rec1_inflows_w_input_points: gpd.GeoDataFrame,
        maf: bool = True,
        ari: Optional[int] = None,
        bound: BoundType = BoundType.MIDDLE) -> gpd.GeoDataFrame:
    """
    Obtain the requested REC1 river inflow scenario data, which can be either Mean Annual Flood (MAF)-based or
    ...
    """
    # Selects and renames specific columns that represent REC1 river inflow data
    rec1_inflow_data = clean_rec1_inflow_data(rec1_inflows_w_input_points)
    if maf:
        ...
        scenario_data = rec1_inflow_data[sel_columns]
        ...
        valid_ari_values = extract_valid_ari_values(rec1_inflow_data)
    ...
    rec1_inflow_scenario_data = scenario_data[sel_columns]
    return rec1_inflow_scenario_data
...
def get_hydrograph_data(
    ...
    #  Obtain the requested REC1 river inflow scenario data
    rec1_inflow_scenario_data = get_rec1_inflow_scenario_data(rec1_inflows_w_input_points, maf, ari, bound)
    ...
    # Generate hydrograph data for the requested REC1 river inflow scenario
    for _, row in rec1_inflow_scenario_data.iterrows():
    ...
    hydrograph_data = gpd.GeoDataFrame(hydro_data_dict, geometry="river_input_point", crs=rec1_inflow_scenario_data.crs)
