...
def data_from_apis(engine,key,base_url,params,geometry_type,source_name, geom):
    ...
    base_url = urllib.parse.urlunparse((parsed_api.scheme, parsed_api.netloc,
                        parsed_api.path.replace("API_KEY",key),"", "", ""))
    """
    According to PsotgreSQL naming convention, a table name must start with a letter or an underscore; 
    the rest of the string can contain letters, digits, and underscores. In our database,
    names of the tables are based on souce_name column which in some cases starts with integer, e.g. "101292-nz-building-outlines",
    ...
    response_data['geometry']= response_data.geometry.set_crs("EPSG:2193",allow_override=True)
...
def access_api_info(engine, source_name): 
    queries = engine.execute("select source_apis,query_dictionary,geometry_col_name,source_name from apilinks \
                             where source_name IN %(source_name)s",({'source_name':source_name}))
    api_record= []
    for query in queries:
        api_record.append(query)
    api_records = pd.DataFrame(api_record, columns=['source_apis','query_dictionary','geometry_col_name','source_name'])
    return api_records
...
# sql = f'select * from region_geometry;'
# eng = create_engine('postgresql+psycopg2://postgres:Pooja@localhost:5432/datasourceapis')

# gdf = gpd.read_postgis(sql, eng, crs='epsg:4326')  
# gdf.to_csv(r'P:\GRI_codes\Digital_Twin_New\src_new\region_geometry.csv')      
